...
def detect_face_batch(images: List[np.ndarray], detector: MTCNN) -> List[Optional[np.ndarray]]:
...
def align_face(image: np.ndarray, landmarks: Optional[np.ndarray], image_size: int) -> Optional[np.ndarray]:
...
def save_image(image: Optional[np.ndarray], input_root: Path, output_root: Path, image_path: Path) -> None:
    if image is None:
